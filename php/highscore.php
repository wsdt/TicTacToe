...
function DB_saveHighscoreEntry($username,$wins, $draws, $losses)
{
    ...
    if (!empty($_POST['username']) && !empty($_POST['wins']) && !empty($_POST['draws']) && !empty($_POST['losses'])) {
        ...
        $wins = strip_tags(mysqli_real_escape_string($tunnel, $wins));
        $draws = strip_tags(mysqli_real_escape_string($tunnel, $draws));
        $losses = strip_tags(mysqli_real_escape_string($tunnel, $losses));
